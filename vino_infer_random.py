...
if __name__ == "__main__":
    ...
    random_model = None
    prev_random = None
    exec_net, input_blob, out_blob = (None,) * 3
    count = 0
    while True:
        if count % 10 == 0:
            random_model = random.choice(filepaths)
            if prev_random is None:
                prev_random = random_model
                sw = 0
            print(f"Choosing random model {random_model}")
            if prev_random != random_model or sw == 0:
                del exec_net
                del input_blob
                del out_blob
